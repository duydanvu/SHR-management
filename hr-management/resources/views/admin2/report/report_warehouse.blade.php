                            ...
                            <td>@foreach($arr as $key_sum => $value_sum )
                                    @if($key_sum == $value->id)
                                        <a type="button" class="btn btn-info " href="{{route('chi_tiet_san_pham_trong_kho',['id'=>$value->id])}}"
                                           data-remote="false" data-toggle="modal" data-target="#modal-chi-tiet-san-pham" style="width: 80px;height: 35px">{{$value_sum}}</a>
                                    @endif
                                @endforeach</td>
    ...
    <script>
        $(document).ready(function(){
            $('#fillter_date').click(function () {
                ...
                $.ajax({
                    type:'POST',
                    url:'{{route('search_list_product_report')}}',
                    data:dt,
